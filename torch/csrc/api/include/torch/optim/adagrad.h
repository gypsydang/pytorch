...
class TORCH_API Adagrad : public Optimizer<AdagradParamState, AdagradParamGroup, AdagradOptions> {
 public:
  explicit Adagrad(std::vector<AdagradParamGroup> param_groups,
      AdagradOptions defaults) : Optimizer<AdagradParamState, AdagradParamGroup, AdagradOptions>(param_groups, defaults) {
      TORCH_CHECK(defaults_.learning_rate() >= 0, "Invalid learning rate: ", defaults_.learning_rate());
      TORCH_CHECK(defaults_.lr_decay() >= 0, "Invalid lr_decay value: ", defaults_.lr_decay());
      TORCH_CHECK(defaults_.weight_decay() >= 0, "Invalid weight_decay value: ", defaults_.weight_decay());
      TORCH_CHECK(defaults_.initial_accumulator_value() >= 0, "Invalid initial_accumulator_value value: ", defaults_.initial_accumulator_value());
      TORCH_CHECK(defaults_.eps() >= 0, "Invalid epsilon value: ", defaults_.eps());
  }

  // TODO: we might want to replace `std::vector<Tensor>` with `ParameterContainer` at some point
  explicit Adagrad(
      std::vector<Tensor> params,
      AdagradOptions defaults) : Adagrad({AdagradParamGroup(params)}, defaults) {}
